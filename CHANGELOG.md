...
- **DSP-001C's safety limiter ran on the wrong signal**: it was applied to the audio stream *before* the master volume multiply, instead of after. Since the limiter's threshold is 0.95, any track whose raw decoded samples reach full scale (1.0 — common; many masters peak at exactly 0 dBFS) had its gain permanently shaved by ~1.3% on every sample, at *every* volume setting, including when the user had the volume turned down far enough that the actual output was never at any risk of clipping. Volume is now applied first; the limiter runs on the real final output, so it only engages when the output could genuinely clip (e.g. from RMS/ReplayGain leveling gain combined with a high volume setting), and is silent otherwise.
