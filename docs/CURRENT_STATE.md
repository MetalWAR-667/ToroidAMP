...
* **RELEASE-BLOCKERS-001**: Ubuntu TTS, User GLSL & Wayland Unified Chassis. (IMPLEMENTED — Windows-validated; Ubuntu manual re-validation pending — this is the v0.667 release gate)
  - TTS intermittency (superseding UBUNTU-WAYLAND-002's mixer-quit/lock mitigation above): pygame.mixer/SDL's Linux device lifecycle remained unreliable across repeated launches even with that mitigation. Voice playback no longer uses pygame.mixer at all — it decodes the synthesized WAV via `soundfile` and plays it through `sounddevice`, the exact backend + device-selection policy (`select_output_device()`) already validated for reliable Ubuntu/PipeWire music playback. `pygame` itself (still required by the CPU visualizers) is untouched; only voice's own `pygame.mixer` dependency was removed.
  - Wayland Playlist/Visualizer centering (superseding UBUNTU-WAYLAND-002's "documented, no portable fix" finding above): confirmed there genuinely is no portable way to position an independent Wayland top-level, so Playlist/Visualizer are now hosted as embedded child widgets inside the chassis's own single top-level window on Wayland specifically (`WindowManager._wayland_embedded`, `UnifiedChassis.embed_module()`, `ModuleShell(embedded=True)`) — Qt's own child-widget layout, not compositor placement, sidestepping the protocol limitation entirely. Windows/X11 keep the existing independent-top-level architecture completely unchanged. Interactive resize of an embedded module is best-effort (not independently verified without a real Wayland compositor); deterministic non-overlapping composition is the delivered guarantee.
  - User shaders reported rendering black in the GLSL Lab on Ubuntu/Mesa/Intel HD 5500 (official shaders unaffected): every sample shader in this repo's `user_shaders/shadertoy/` was verified end-to-end (compile, link, and non-black render) against a real desktop OpenGL driver here, finding no pipeline defect in `load_shader_file()`/`classify_and_wrap_source()`/`paintGL()`'s uniform binding for these files — not reproducible on this cut's Windows environment. Added a diagnostic (`lab_app.py::_diagnose_black_render`) that samples the actual rendered pixels once after a successful load and logs a clear warning if a shader compiled/linked fine (`ok=True`) but still painted all-black — something neither GL nor the existing compile/link error surfacing catches — so the next real-hardware reproduction yields an actionable signal instead of a silent mystery.
