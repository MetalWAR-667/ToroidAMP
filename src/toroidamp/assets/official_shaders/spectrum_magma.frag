...
// ------------------------------------------------------------------
// 3. Radial spectrum equalizer crown + energy beams + sparks
// ------------------------------------------------------------------
vec3 radial_spectrum_crown(vec2 uv, float t, float bass, float mids, float treble,
                           float peakHold, float beatPulse, float energy) {
    vec3 col = vec3(0.0);
    float r = length(uv);
    ...
    // 1. Radial 64-Band Circular Equalizer Columns
    float angleNorm = a / 6.2831853;
    float binFloat = angleNorm * 63.0;
    int i0 = int(floor(binFloat));
    int i1 = min(i0 + 1, 63);
    float specVal = mix(taSpectrum[i0], taSpectrum[i1], fract(binFloat));
    
    // Equalization curve for outer crown
    float crownBase = 0.52 + bass * 0.18;
    float crownHeight = specVal * 0.45 * u_taper * (1.0 + treble * 0.8);
    
    // Segmented bar geometry along polar ray
    float barGap = sin(angleNorm * 64.0 * 3.14159);
    float barMask = smoothstep(0.18, 0.45, abs(barGap));
    
    float distToCrown = r - crownBase;
    if (distToCrown > 0.0 && distToCrown < crownHeight && barMask > 0.0) {
        float segFrac = distToCrown / max(0.01, crownHeight);
        float segStripe = sin(distToCrown * 80.0);
        float segMask = smoothstep(0.1, 0.5, segStripe);
        
        vec3 colA = vec3(0.0, 0.95, 1.0); // Cyan
        vec3 colB = vec3(1.0, 0.85, 0.1); // Gold
        vec3 colC = vec3(1.0, 0.05, 0.6); // Hot Magenta
        
        vec3 barCol = mix(colA, colB, smoothstep(0.0, 0.5, segFrac));
        barCol = mix(barCol, colC, smoothstep(0.5, 1.0, segFrac));
        
        col += barCol * segMask * (1.4 + treble * 1.2 + peakHold * 0.8) * barMask;
    }
    
    // Floating peak dot along crown
    float peakRadius = crownBase + specVal * 0.50 * u_taper;
    float peakDist = abs(r - peakRadius);
    if (peakDist < 0.012 && barMask > 0.0 && specVal > 0.05) {
        col += vec3(1.0, 1.0, 1.0) * smoothstep(0.012, 0.0, peakDist) * 1.8;
    }

    // 2. Radial energy beams
...
void main() {
    ...
    // Additive composition of the three layers with balanced weights
    vec3 col = plasma_background(uv, t, bass, mids, treble, energy) * 0.7;
    col += torus_layer(uv, t, bass, mids, treble, peakHold, beatPulse, energy) * 0.65;
    col += radial_spectrum_crown(uv, t, bass, mids, treble, peakHold, beatPulse, energy) * 0.65;

    // Center glow core: cold<->warm by energy.
    float core = 0.035 / (0.04 + length(uv));
    vec3 coreCol = mix(vec3(0.20, 0.60, 1.00), vec3(1.00, 0.45, 0.15), energy);
    col += coreCol * core * (0.4 + bass * 0.5);

    // Soft vignette to hold composition + corner falloff.
    col *= 1.0 - 0.35 * smoothstep(0.7, 1.5, length(uv));

    // Master energy scale
    col *= (0.35 + taRms * 0.65) * (0.8 + energy * 0.3);
    
    // Soft-shoulder tone mapping to eliminate blown-out whites and harsh clipping
